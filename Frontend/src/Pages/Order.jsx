...
const Order = () => {
  ...
  const filteredItem = flavor.filter((item) => {
    return item.name.toLowerCase().includes(userInput.trim().toLowerCase());
  });
  ...
  return (
    <div className="min-h-screen bg-[#FFF8F3] py-12 px-6">
      ...
      <div>
        <input
          className="w-full bg-gray-300 h-12 rounded-md px-10"
          type="text"
      ...
      <div className="sticky flex justify-end left-10 right-3 bottom-1 top-36 z-50">
        <Link to="/cart">
          <button className="w-36 mt-10 bg-[#541F03] text-white h-12 rounded-lg">
            Cart
          </button>
        </Link>
      </div>
      ...
      {userInput.trim() !== "" ? (
        filteredItem.length > 0 ? (
          ...
          <h1 className="text-center text-2xl mt-10">No flavor found 😔</h1>
        )
      ) : (
        <>
          {/* Ice Cream */}
